      ...
      <div className="container bins-container-centered">
        ...
        <div className="outer-bins-card">
          ...
          <div className="row justify-content-center gx-3 gy-4">
            {bins.map((bin) => {
              // Support both: levels[bin.key] is a number OR an object
              const levelData = levels?.[bin.key];
              const displayLevel =
                typeof levelData === 'number'
                  ? Math.round(levelData)
                  : typeof levelData?.level_percentage === 'number'
                  ? Math.round(levelData.level_percentage)
                  : null;
              const alerts =
                Array.isArray(levelData?.alerts) ? levelData.alerts : [];
