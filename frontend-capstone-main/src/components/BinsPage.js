  ...
  return (
    <div className="bins-page">
      <div className="container bins-container-centered">
        ...
        {/* Render per-bin alerts */}
        {levels &&
          bins.map(bin => {
            const binAlerts = levels[bin.key]?.alerts ?? [];
            return binAlerts.map((alert, idx) => (
        ...
        <div className="row justify-content-center gx-3 gy-4">
          {bins.map(bin => {
            ...
            const displayLevel = lvlObj?.level_percentage ?? null; // Use DB value directly
